...
> **Instruction file structure**: `AGENTS.md` (this file) is the single source of truth, read by all AI agents (Claude Code, Cursor, Copilot, Codex, …). `CLAUDE.md` just imports it via `@AGENTS.md`. Detailed rules live in `.claude/rules/` — when adding new patterns, edit the appropriate rules file, NOT this root file. Route by topic:
>
...
components/
├── domains/              # Feature components (account, dashboard, messaging, notifications, payment, posts, subscriptions, tips, users)
...
lib/
├── animations.ts         # Motion variants (stepSlideVariants, etc.)
